...
//      }
      ...
      for (int y = 0; y < ncoly; y++) {
        //Calculate standard deviation of y
        sigmay[y] = Math.sqrt(Math.abs(cvs._denomy[y][y] / fry.numRows() - 1));
        //Calculate standard deviation of x
        sigmax[y] = Math.sqrt(Math.abs(cvs._denomx[y][y] / frx.numRows() - 1));
      }
      ...
      Key<Vec>[] keys = Vec.VectorGroup.VG_LEN1.addVecs(ncoly);
      for (int y = 0; y < ncoly; y++) {
        res[y] = Vec.makeVec(ArrayUtils.div(ArrayUtils.div(cvs._cor[y], (fry.numRows() - 1)), denom[y]), keys[y]);
      }
      return new ValFrame(new Frame(fry._names, res));
    }
  }
